<%-- 
    Document   : SHOWPRODUCT
    Created on : Aug 31, 2018, 2:56:51 PM
    Author     : Bajaja
--%>


<%@page contentType="text/html" pageEncoding="UTF-8"%><!-- UTF stands for UCS Transformation Format, UCS stands for Unified Computing System developed by Cisco. The size of character in UTF-8 may be 1 byte to 4 bytes. -->
<!DOCTYPE html>
<html>
<head>
    <title>Show Product</title>
<link rel="stylesheet" href="ShowProduct.css">
<script defer src="https://use.fontawesome.com/releases/v5.2.0/js/all.js" integrity="sha384-4oV5EgaV02iISL2ban6c/RmotsABqE4yZxZLcYMAdG7FAPsyHYAPpywE9PJo+Khy" crossorigin="anonymous"></script>
</head>
<body a link="white" vlink="white"><!-- a link(active link) and v link(visited link) --> 

    <div>
        <%
          if(session.getAttribute("CustomerID")==null) //if customer wants to open this page without login they will not access. 
              response.sendRedirect("login.jsp");
            
        %>
    </div>
    
<div class="Home">
<table>
<tr>
    <td><a href="index.jsp"><image src="images/logo.jpg" width="260" height="120"/></a></td>
<td>
<ul>
<li><a href="Collection.jsp">COLLECTIONS</a></li>
<li><a href="about.jsp">ABOUT AB</a></li>
<li><a href="category.jsp">WATCH CATEGORY</a></li>
<li><a href="Contact.jsp">CONTACT US</a></li>
<li><a href="search.jsp"><i class="fas fa-search"/></i></a></li>
<li><a href="ViewCart.jsp"><i class="far fa-heart"></i></a></li>
<li><a href="#"><%= session.getAttribute("Custname") %> </a>
    <ul>
        <li><a href="Profile.jsp">MY PROFILE</a></li>
        <li><a href="MyOrder.jsp">MY ORDER'S</a></li>
        <li><a href="ViewCart.jsp">WISHLIST</a></li>
        <li><a href="Notification.jsp">NOTIFICATION</a></li>
        <li><a href="Logout.jsp">LOGOUT</a></li>
    </ul>
</li>

</ul>
</td>
</tr>
</table>
</div>
    
    <%@page import="java.io.* , java.sql.*" %>               <!-- this is java header file -->    
        <%!                                                 //this is scriptlet tag for jsp code only
            String Asin; ResultSet rs;          %> 
        <%
            try
            {
                
              Asin=request.getParameter("ASIN");
              
              Class.forName("oracle.jdbc.driver.OracleDriver");         //driver name
              Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","watchinfo" ,"watchinfo");       //database connection
              String qry="select * from ADDPRODUCT" + " where ASIN=?";      //get the values from database trhrough this query
              PreparedStatement ps=con.prepareStatement(qry);
              
              ps.setString(1, Asin);
              
              rs=ps.executeQuery();       //through this we execute query
              rs.next();
              
              
            }

            catch(Exception ex){
                  out.println(ex);
              }
%>


    <div class="IMAGE">
        <image src="images\image10.jpg" height="700" width="500"/>
    </div>
 <div class="addproduct">
        <h1>PRODUCT DESCRIPTION</h1>
        <form action="#">
            <p id="p">ASIN:  &ensp;&ensp;&ensp;<ps><%= rs.getString(1) %></ps><hr></p>
            <p>Product Title: &ensp;&ensp;&ensp;
            <ps><%= rs.getString(2)  %></ps><hr></p>
            <p>Price: &ensp;&ensp;&ensp;
            <ps><%= rs.getString(3)  %></ps><hr> </p>
	    <p>Occasion: &ensp;&ensp;&ensp;
            <ps><%= rs.getString(4)  %></ps><hr> </p>
	    <p>Watch Type: &ensp;&ensp;&ensp;		
            <ps><%= rs.getString(5)  %></ps><hr> </p>
            <p>Pack Of: &ensp;&ensp;&ensp;
            <ps><%= rs.getString(6)  %></ps><hr> </p>
	    <p>Warranty Summary: &ensp;&ensp;&ensp;		
            <ps><%= rs.getString(7)  %></ps><hr> </p>
	    <p>Country of Origin: &ensp;&ensp;&ensp;		
            <ps><%= rs.getString(8)  %></ps><hr> </p>
            <p>Description: &ensp;&ensp;&ensp;		
            <ps><%= rs.getString(9)  %></ps><hr> </p>
            <br /><br />
            <input type="submit" name="" value="BUY NOW"/><br /><br />
            <input type="submit" name="" value="ADD TO CART"/>
            
        </form>
        
    </div>

<div class="footer">

        <div class="outer">
            <div class="inner">
                <h4><u>AB COLLECTIONS</u></h4>
                <a href="Men.jsp">Men's</a>
                <a href="Women.jsp">Women's</a>
                <a href="Kid.jsp">Kid's</a>
                <a href="Collection.jsp">All our Watches</a>
		</div>
            
            <div class="inner">
                <h4><u>TERMS AND CONDITION</u></h4>
                <a href="Privacy.jsp">Privacy Notice</a>
                <a href="Term.jsp">Terms of Use</a>
            </div>
            <div class="inner2">
                <h4><u>CUSTOMER SERVICE</u></h4>
                <a href="MAP.jsp">Find a Service Center</a>
                <a href="Collection.jsp">Online Catalogue</a>
                <a href="Service.jsp">Service Prices</a>
            </div>
            <div class="inner2">
                <h4><u>FOLLOW US</u></h4>
                <a href="www.facebook.com">Facebook</a>
                <a href="https://twitter.com/login?lang=en">Twitter</a>
                <a href="www.youtube.com">Youtube</a>
            </div>
        </div>

        <hr>
        <div class="copyright">
            <p>Copyright by AB Collection © 2018</p>
        </div>


    </body>
</html>